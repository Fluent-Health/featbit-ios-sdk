...
final class TrackInsightTests: XCTestCase {
    ...
    private func makeTracker() -> HttpTrackInsight {
        let options = FBOptions.Builder("secret").event("https://events.example.com").build()
        return HttpTrackInsight(options: options, session: MockURLProtocol.session())
    }

    func testEvaluationInsightPostsSingleElementArray() async {
        ...
        await makeTracker().run(.forEvaluation(user: user, flag: flag, timestamp: 999))
    ...
    func testIdentifyInsightHasEmptyVariations() async {
        MockURLProtocol.handler = { _ in (200, Data()) }
        await makeTracker().run(.forIdentify(user: FBUser.builder("u2").build()))
